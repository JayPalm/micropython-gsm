    ...
    def getGPS(self):
        ...
        if int(a[0]) == 1 and int(a[1]) == 1:
            return gpsinfo(
                ...
                speed=float(a[6])
                if a[6] != ""
                else float(0),  # Speed Over Ground. Unit is knots.
                alt=float(a[5])
                if a[5] != ""
                else float(0),  # MSL Altitude. Unit is meters
                vsat=int(a[14]) if a[14] != "" else int(0),  # GNSS Satellites in View
                usat=int(a[16]) if a[16] != "" else int(0),  # GNSS Satellites Used
                accuracy=float(a[10])
                if a[10] != ""
                else float(0),  # Horizontal Dilution Of Precision
                ...
                second=int(a[2][12:14])
                if a[2] != ""
                else int(0),  # 6 digit second with subseconds
            )
        return None
